...
module.exports = {
  ...
  getRecentImagesUrl: function(tagName,callback){
    console.log("@instalib.getRecentImagesUrl - for:",tagName);
    Instagram.tags.recent({
      name: tagName,
      error: function(errorMessage, errorObject, caller){
        console.log("@instalib.getRecentImagesUrl - getting tag '%s' returned an error",tagName,errorMessage,errorObject);
        return errorObject;
      },
      complete: function(data){
        // console.log("@instalib.getRecentImagesUrl - complete got data:",data)
        var imagesUrl = [];
        ...
        // console.log("@instalib.getRecentImagesUrl - urls are %s:",imagesUrl);
        callback(imagesUrl);
      }
    })
  },
  getRecentImageUrl: function(tagName,callback){
    console.log("@instalib.getRecentImageUrl - for:",tagName);
    Instagram.tags.recent({
      name: tagName,
      error: function(errorMessage, errorObject, caller){
      ...
      complete: function(data){
        // console.log("@instalib.getRecentImageUrl - complete got data:",data)
        var latestTime = 0;
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (currentTime > latestTime) {
            ...
            if(data[i].images[resolution].url){
              ...
              latestUrl = data[i].images[resolution]
            }
          };
        };
        // maybe the image's width and height should be passed on too?
        console.log("@instalib.getRecentImageUrl - url is %s:",latestUrl);
        callback(latestUrl);
      }
    })
  },
